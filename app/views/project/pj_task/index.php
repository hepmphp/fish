...
<script>
    ...
    function ajax_list(param) {
        ...
        var list_html = '';
        $.getJSON('/api/project/task/get_list/?' + $.param(param), function (data) {
            layer.closeAll();
            if (data.status == 0) {
...
var urls = {
    create_url:'/api/project/task/create',
    update_url:'/api/project/task/update',
    delete_url:'/api/project/task/delete',
    info_url:'/api/project/task/info'
};
 ...
 */
function add(){
    var url = '/project/task/create';
    layer_form(url,1,['600px', '400px']);
}
/**
 * 修改
 * @param id
 */
function edit(id) {
    var url = "/project/task/update?id="+id;
