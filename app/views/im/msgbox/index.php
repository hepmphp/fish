    ...
    <div class="form-item">
        ...
        <option value="">请选择</option>
          <?php
            ...
            <input placeholder="文本" class="form-control" name="content" id="content" value="<?=$form['content']?>" type="text">
        </div>
        <div class="form-group">
            <label class="control-label">发送消息时间：</label>
...
<script>
    ...
    function search_list(){
                    var search_param= {
						...
						remark: $('#remark').val(),
						content: $('#content').val(),
						friend_group_id: $('#friend_group_id').val(),
...
'<td>[delete_time]</td>'+
        ...
        var list_html = '';
        $.getJSON('/api/im/msgbox/get_list/?' + $.param(param), function (data) {
            layer.closeAll();
            if (data.status == 0) {
...
replace('[remark]', d.remark).
replace('[content]', d.content).
replace('[friend_group_id]', d.friend_group_id).
replace('[group_id]', d.group_id).
replace('[send_time]', d.send_time).
replace('[read_time]', d.read_time).
...
var urls = {
    create_url:'/api/im/msgbox/create',
    update_url:'/api/im/msgbox/update',
    delete_url:'/api/im/msgbox/delete',
    info_url:'/api/im/msgbox/info'
};
 ...
 */
function edit(id) {
    var url = "/im/msgbox/update?id="+id;
    ...
    layer.open({
        ...
        yes: function(index, layero){
            ...
            var param ={
					...
					friend_group_id:body.find('#friend_group_id').val(),
