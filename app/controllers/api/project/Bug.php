...
class Bug extends BaseController{
    ...
    public function get_search_where(){
        ...
        $bug_level = Input::get_post('bug_level',0,'intval');
        if($bug_level){
            if(!Validate::required('bug_level')){
                throw  new  LogicException(-1,'链接名称');
            }
            $where['bug_level'] = $bug_level;
        }
    ...
    public function create(){
        ...
        $this->pj_bug->create($form);

    }

    public function update(){
        $form = $this->get_search_where();
        $this->pj_bug->save($form);
    }
    ...
    public function get_list(){
        ...
        list($res,$total) = $this->pj_bug->get_list_info($where,$page,$per_page,'*');
        $data['list'] = $res;
        $data['total'] = $total;
        $data['page'] =$page;
