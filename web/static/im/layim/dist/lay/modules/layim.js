...
layui.define(['layer', 'laytpl', 'upload'], function (exports) {
    ...
    //添加好友分组
    LAYIM.prototype.addFriendGroup = function (data) {
        return addFriendGroup(data),this;
    };

    //删除好友分组
    LAYIM.prototype.removeFriendGroup = function (data) {
        return removeFriendGroup(data),this;
    };
    ...
    //添加好友分组
    var addFriendGroup = function (data) {
        if(!data) {
            return;
        }
        // 添加到cache
        cache.friend.push(data);
        data.groupname = data.groupname || '未命名分组';
        //添加HTML
        var html = '<li>' +
            '<h5 layim-event="spread" class="layim-friendgroup'+data.id+'" data-id="'+data.id+'" lay-type="true">' +
            ...
            '</ul>' +
            '</li>';
        layimMain.find(".layim-list-friend").append(html);
    };

    //删除好友分组
    var removeFriendGroup = function (data) {
        if (!data) {
            return;
        }
        //删除cache里面对应的数据
        var j, len;
        for (j = 0 , len = cache.friend.length; parseInt(data.id) !== parseInt(cache.friend[j].id) && j < len; j++) {}
        if (parseInt(data.id) !== parseInt(cache.friend[j].id)) {
            layer.msg('删除失败，请刷新页面重试！');
            return;
        }
        cache.friend.splice(j, 1);
        // 移除元素
        var element = layimMain.find(".layim-list-friend .layim-friendgroup" + data.id).parent();
        if (element.length === 0) {
            layer.msg('删除失败，请刷新页面重试！');
            return;
        }
