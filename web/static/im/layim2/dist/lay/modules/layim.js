...
;layui.define(["layer", "laytpl", "upload"], function (i) {
    ...
    var u = function (i) {
        ...
        h = ['<li  data-name="{{d.username}}" {{ d.mine ? "class=layim-chat-mine" : "class=layim-chat-to" }} {{# if(d.cid){ }}data-cid="{{d.cid}}"{{# } }}>', '<div class="layim-chat-user" data-id="{{d.id}}" data-username="{{d.username}}"><img src="{{ d.avatar }}"><cite>', "{{# if(d.mine){ }}", '<i>{{ layui.data.date(d.timestamp) }}</i>{{ d.username||"佚名" }}', "{{# } else { }}", '{{ d.username||"佚名" }}<i>{{ layui.data.date(d.timestamp) }}</i>', "{{# } }}", "</cite></div>", '<div  class="layim-chat-text">{{ layui.data.content(d.content||"&nbsp") }}</div>', "</li>"].join(""),
    ...
    var x, b, w, k, C, S = function (i, a, n) {
        ...
        status: function (i, a) {
            ...
            var a = _();
            var from_user = i.parent().parent().parent().find('.layim-chat-main .layim-chat-mine').eq(0);
            var to_username = from_user.data('name');
            return j.base.chatLog ? (t.close(ti.chatLog.index), ti.chatLog.index = t.open({
                ...
                content: j.base.chatLog + "?id=" + a.data.id + "&type=" + a.data.type+"&to_username="+to_username
            })) : t.msg("未开启更多聊天记录")
